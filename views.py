...
def consulta_membro(request):
    if request.method == 'POST':
        ...
        membro =  Membro.objects.filter(nome_comp__contains=partenome)
...
def consultar_evento(request):
    if request.method == 'POST':
        ...
        Evento.objects.filter(inicio=evento_inicio)
        #falta passar a lista pra uma pag e exibir la, mas ta pegando ela pelo menos :) 
    else:
...
def consultar_aconselhamento(request):
    if request.method == 'POST':
        ...
        Aconselhamento.objects.filter(inicio=aconselhamento_inicio)
        #falta passar a lista pra uma pag e exibir la, mas ta pegando ela pelo menos :) 
    else:
